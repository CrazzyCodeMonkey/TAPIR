	...
	<table class="table shadow-z-2">
		...
		<cfoutput query="attributes.reviews">
			...
			<tr id="#rev.reviewId#">
				...
				<td>
					<span class="large"><i class="glyphicon glyphicon-user"></i></span>
					<br />
				...
				<td>
					<cfif attributes.which=="NEW">
						...
						<cfif rev.status=="WARNING">
							...
							<cfset fireScale = (fireScale<1?fireScale:1)>
						...
						<cfif attributes.which=="COMPLETED">
							<cfif (rev.ticket!=0)>
								<cfif (rev.childreview!="")>
									<a href="##" class="submitInput" data-child="#rev.childreview#"><i class="glyphicon glyphicon-chevron-up"></i></a>
								<cfelse>
									<a href="##" class="submitInput" data-location="/tapir/rpc/review.cfc?method=reviewAdd&reviewId=#request.tapir.getReviewId()#" data-data='{"_sPrevReview":"#rev.reviewId#"}'><i class="glyphicon glyphicon-plus"></i></a>
								</cfif>
							<cfelse>
								<i class="glyphicon glyphicon-minus"></i>
							</cfif>
						<cfelse>
							<a href="##" class="submitInput" data-modal="/tapir/rpc/review.cfc?method=reviewReminder&reviewId=#request.tapir.getReviewId()#" data-data='{"_reviewId":"#rev.reviewId#"}'><i class="glyphicon glyphicon-envelope"></i></a>
						</cfif>
					</span>
					<br />
					<small>Parent&nbsp;&nbsp;/&nbsp;&nbsp;<cfif attributes.which=="COMPLETED">Child<cfelse>Remind</cfif></small>
				</td>
				<td>
					<cfif (attributes.which=="COMPLETED")>
						<a href="##" class="submitInput" data-reload="/tapir/rpc/review.cfc?method=reviewReOpen&reviewId=#request.tapir.getReviewId()#" data-data='{"_reviewId":"#rev.reviewId#"}'>
							<span class="large"><i class="glyphicon glyphicon-retweet"></i></span>
							<br />
							Reopen
						</a>
					<cfelse>
						<a href="##" class="submitInput" data-modal="/tapir/rpc/review.cfc?method=reviewReassignPrompt&reviewId=#request.tapir.getReviewId()#" data-data='{"_reviewId":"#rev.reviewId#"}'>
							<span class="large"><i class="glyphicon glyphicon-random"></i></span><br />Reassign
						</a>
					</cfif>
				</td>
